...
class CodeGenerationAgent:
    def __init__(self, name: str = "DeadlyAccurateAgent"):
        ...
        self.average_complexity: float = 0.0
    ...
    def _calculate_complexity(self, requirements: str) -> int:
        """Calculate complexity score (0-100 scale) with balanced analysis."""
        score = 20  # Base score
        ...
        # Technical complexity indicators with reasonable weights
        technical_terms = {
            'api': 3, 'database': 4, 'authentication': 5, 'security': 4,
            'async': 4, 'concurrency': 5, 'threading': 4, 'parallel': 5,
            'microservices': 8, 'distributed': 8, 'kubernetes': 7, 'docker': 6,
            'machine learning': 10, 'ai': 8, 'real-time': 7, 'streaming': 6,
            'testing': 3, 'monitoring': 3, 'fastapi': 5, 'django': 5
        }

        tech_score = sum(weight for term, weight in technical_terms.items()
                        if term in requirements.lower())
        score += min(tech_score, 30)

        # Architectural complexity bonuses
        if any(term in requirements.lower() for term in ['microservices', 'distributed']):
            score += 15
        if any(term in requirements.lower() for term in ['machine learning', 'ai']):
            score += 10
        if any(term in requirements.lower() for term in ['security', 'authentication']):
            score += 8
        if any(term in requirements.lower() for term in ['real-time', 'streaming']):
            score += 6

        # Length bonus for complex requirements
        if length > 300:
...
'''
    ...
    def _generate_intelligent_components(self, analysis: Dict[str, Any], code_type: str,
                                        architecture: str, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
...
class BusinessLogicError(ApplicationException):
    """Raised for business logic failures"""
    pass

class IntegrationError(ApplicationException):
    """Raised for integration failures"""
    pass
...
'''
    ...
    def _validate_code(self, code: str) -> Dict[str, Union[bool, str, int, float]]:
...
'''
        ...
        # Exception classes are now defined globally at module level
